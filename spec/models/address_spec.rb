...
describe Address do
  ...
  describe "to_s" do
    let(:address) { FactoryGirl.build(:address, street: '34 rue de la gare', city: 'Marseille', postal_code: '13003', more_info: 'près du piano, à coté du McDo', country: 'FR', kind: 'Work') }
    it { expect(address.to_s).to eq("#{address.full_address} [Work]") }
  end
end
